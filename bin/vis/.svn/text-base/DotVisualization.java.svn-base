...
public abstract class DotVisualization extends GraphVisualization {
	 ...
	 * CONSTRUCTOR
	 */
	protected DotVisualization() {
		graphType = UNDIRECTED;
		algorithm = DOT;
		isEdgeVisible = true;
		buffer = new StringBuffer();
		inputFile = System.getProperty("output-path") + "G.dot";
		outputFile = System.getProperty("output-path") + "G.png";
	}
	...
	public void setAlgorithm(String algorithm) {
		if (ALL_ALGORITHMS.indexOf(algorithm) > 0)
	...
	public void visualize(Graph graph) {
		...
		try {
			String cmd = algorithm;
			cmd += " -T" + OUTPUT_FORMAT + " " + inputFile + " -o "
					+ outputFile;
			System.out.println(cmd);
			Runtime.getRuntime().exec(cmd + NEW_LINE);
		} catch (IOException ioe) {
			ioe.printStackTrace();
		}
